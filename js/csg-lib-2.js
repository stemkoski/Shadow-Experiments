...
class Vertex 
{
    constructor(pos, uv=null, color=null) 
    {
        ...
        // optional data vectors (copy if present)  
        if (uv != null)
    ...
    // TODO: shouldn't this just affect the plane?
...
// static method to create a Plane object, given three points on that plane
// direction of normal: assume points in counterclockwise order, use right hand rule
...
class Polygon 
{
    // vertices is an array of at least three coplanar points, 
    //   arranged in (counter/clockwise?) order when viewed from above (defined by normal vectors?)
    ...
    // assigns the unit normal vector from the plane to each vertex.

    // ??? used in CSG calculations? -- doesn't seem so.
    //  replace with new data when converted to a mesh?
    computeFaceNormals()
    {
        for (let i = 0; i < this.vertices.length; i++)
        {
            this.vertices[i].normal = this.plane.normal.clone();
        }
    }
    ...
    flip() 
    {
        this.vertices.reverse().forEach(v=>v.flip()) // flips vertex normals, not important?
        this.plane.flip();
    }
}
